...
public class SankeyServiceTest {
    ...
    public void groupedRelationToValues() {
        ...
        map = service.groupedRelationToValues(smallLog.get(0), "Activity", "COUNT", groups, map, smallLog.getClassifiers().get(0), false);
    ...
    public void findTraceGroups() {
        ...
        assertEquals(2, groups.map.size());
        List<TraceGroup> zeroIndexGroups = groups.map.get(0);
        List<TraceGroup> zeroAndFirstIndexGroups = groups.map.get(1);

        assertEquals(1, zeroIndexGroups.size());
        assertEquals(1, zeroAndFirstIndexGroups.size());


        assertEquals(zeroIndexGroups.get(0).traces.size(), 2);
        assertEquals(zeroAndFirstIndexGroups.get(0).traces.size(), 2);
    ...
    public void groupBynthActivity() {
        ...
        traceGroup.traces.add(smallLog.get(0));
        traceGroup.traces.add(smallLog.get(1));
        traceGroup.index = 0;
        List<TraceGroup> traceGroups = service.groupBynthActivity(traceGroup, "Activity", 1, smallLog.getClassifiers().get(0));
        assertEquals(1, traceGroups.size());

        List<TraceGroup> traceGroups2  = service.groupBynthActivity(traceGroup, "Activity", 2, smallLog.getClassifiers().get(0));
