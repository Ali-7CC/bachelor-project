...
public class TraceGroupsMap {
    ...
    /**
     * Constructs an string for an activity using the given trace and activity's position.
    ...
    public String makeGroupName(XTrace trace, String attrKey, int position) {
        ...
        if (!map.containsKey(position)) return groupName;
        else {
            // Get the trace groups
            List<TraceGroup> traceGroups = map.get(position);
            for (TraceGroup traceGroup : traceGroups) {
                // Checks whether the given trace is part of the groups that share these activities
