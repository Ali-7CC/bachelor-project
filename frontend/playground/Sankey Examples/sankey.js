...
const updateNames = (namesGroup, generatedData) => {
  const updateNames = namesGroup
    ...
    .text((d) => d.name.split("_")[0] + " : " + d.value);
...
const updateLinksGroup = (linksGroup, generatedData) => {
  // Joining new groups
  const links = linksGroup.selectAll("g").data(generatedData.links).join("g");
  ...
  links
    ...
    .attr("stroke", (d) => {
      ...
      console.log(option);
      if (option === "input") {
        return colorScale(d.source.name.split("_")[0]);
      } else {
        return colorScale(d.target.name.split("_")[0]);
      }
    })
      ...
      colorDropDown.addEventListener("change", (e) => {
        ...
        if (option === "input") {
          updatedLinks
            ...
            .transition()
            .duration(1000)
            .attr("stroke", (d) => colorScale(d.source.name.split("_")[0]));
        } else {
          updatedLinks
            .selectAll("path")
            .transition()
            .duration(1000)
            .attr("stroke", (d) => colorScale(d.target.name.split("_")[0]));
        }
      });
